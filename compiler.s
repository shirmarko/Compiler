...
; %macro SHIFT_OPT 3 ; %1 = size of frame (constant) %2 = ;r11 holds the address of first arg to be copied
; 	; %3 = r10 holds the prev rbp - where we need to put the new frame
...
%macro SHIFT_FRAME 1 ; %1 = size of frame (constant)
	...
	%rep %1
		dec rax
		mov rsi, [rbp-WORD_SIZE*i]
		mov [rbp+WORD_SIZE*rax], rsi
		%assign i i+1
	%endrep
	pop rsi
	pop rax
...
%macro SHIFT_FRAME_REG 1 ; %1 = size of frame (constant)
	; push rax
	; push rsi
	; push rcx
	; push r10
	...
	je %%End_Loop
		...
		shl r9, 3
		mov r11, rbp
		sub r11, r9 					;[rbp-WORD_SIZE*rbx]
		mov rdi, [r11]
		mov [rbp+WORD_SIZE*rax], rdi
		inc rcx
		inc rbx
		; %assign i i+1
	jmp %%Loop
	...
	shl rax, 3			;rbp+(8*rax) holds the place we need to put the new rsp
	add rax, rbp
	mov rsp, rax
