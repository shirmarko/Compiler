...
(define fold-left
	(lambda (func acc curr)
		(if (null? curr)
			acc
			(fold-left func (func acc (car curr)) (cdr curr))
		)
	)
)
...
(define fold-right
  (lambda (func acc curr)
    (if (null? curr)
        acc
        (func (car curr) (fold-right func acc (cdr curr))))))


(define cons*
	(lambda x
		(let ((t (fold-right cons x '())))
		(remove-nil t)))
)
